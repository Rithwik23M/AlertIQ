...
// Only these statuses appear in the investigations view
const INVESTIGATION_STATUSES: AlertStatus[] = [
  "in_progress",
  "escalated",
...
export function InvestigationsClient() {
  ...
  useEffect(() => {
    ...
    Promise.all(
      INVESTIGATION_STATUSES.map((status) =>
        fetchAlerts({ status, sort_by: "risk_score", sort_dir: "desc", page: 1, page_size: 50 })
      )
    )
      .then(([escalated, review, inProgress]) => {
        setGroups({
  ...
  const total =
    groups.escalated.length +
    groups.needs_further_review.length +
      ...
      {!loading && !error && total === 0 && (
        <div className="rounded border border-gray-200 bg-white shadow-sm px-6 py-16 text-center">
          ...
          <p className="text-xs text-aq-text-dim">
            ...
            <a href="/" className="text-aq-accent hover:underline">
              Queue
            </a>{" "}
      ...
      {!loading && !error && total > 0 && (
        <div className="space-y-8">
          <AlertGroup
            title="Escalated"
            color="red"
            alerts={groups.escalated}
            onRowClick={(id) => router.push(`/alerts/${encodeURIComponent(id)}`)}
          />
          <AlertGroup
            title="Needs Further Review"
            color="amber"
            alerts={groups.needs_further_review}
            onRowClick={(id) => router.push(`/alerts/${encodeURIComponent(id)}`)}
          />
          <AlertGroup
            title="In Progress"
            color="violet"
            alerts={groups.in_progress}
            onRowClick={(id) => router.push(`/alerts/${encodeURIComponent(id)}`)}
          />
        </div>
      )}
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 mb-3">
        <span className={`inline-block w-2.5 h-2.5 rounded-full ${DOT_MAP[color]}`} />
          ...
          <tbody className="divide-y divide-aq-border bg-white">
            {alerts.map((alert, idx) => (
              ...
              >
                ...
                <td className={tdCls}>
                  <div className="text-xs font-medium text-aq-text max-w-[10rem] truncate" title={alert.rule_name}>
                    {alert.rule_name}
                  </div>
